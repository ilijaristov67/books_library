...
if (!isset($data['id'])) {
    $errors[] = ['error' => true, 'message' => 'ID parameter is missing or empty'];
}
...
if ($book->editBook($id, $title, $number_of_pages, $author_id, $published, $photo, $category_id)) {
    echo json_encode(['success' => true, 'message' => 'Book Successfully Edited']);
} else {
    echo json_encode(['error' => true, 'message' => 'Book Was Not Edited']);
}
